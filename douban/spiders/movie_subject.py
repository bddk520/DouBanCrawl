...
class MovieSubjectSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        movie['type'] = selector.xpath(
            '//*[@id="info"]/span[5]/text()').extract_first()
        movie['directors'] = selector.xpath(
            '//*[@id="info"]/span[1]/span[2]/a/text()').extract_first()
        movie['duration'] = selector.xpath(
            '//*[@id="info"]/span[13]/text()').extract_first()
        movie['douban_score'] = selector.xpath(
            '//*[@id="interest_sectl"]/div[1]/div[2]/strong/text()').extract_first()
        print(movie['title'])
        yield movie
